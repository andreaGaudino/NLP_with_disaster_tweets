...
Our experimental evaluation yielded several critical insights regarding the interplay between parameter capacity, regularization, and data augmentation in domain-specific language models: Our baseline BERTweet model was fully fine-tuned for 3 epochs at a learning rate of \texttt{lr=2e-5} with warmup~\cite{devlin2019bert}, utilizing all $\sim$135 million parameters to achieve a Kaggle F1-score of 0.839. In contrast, by configuring LoRA~\cite{hu2021lora} with a rank of $r=8$, we reduced the trainable parameter footprint to just 887,042 weights (0.65\% of the total model). Ultimately, this parameter-efficient strategy proved highly successful; the BERTweet model combined with LoRA achieved a peak Kaggle F1-score of 0.847, yielding superior performance compared to traditional full fine-tuning while drastically minimizing computational costs. While our DoRA~\cite{liu2024dora} implementation (rank $r=16$) also achieved a highly competitive Kaggle F1-score of 0.841, it was slightly outperformed by the simpler LoRA framework. This result aligns with findings that on specific domains, the simplicity of LoRA can act as a more effective regularizer, whereas DoRA's magnitude component may require more training epochs or higher-signal data to fully realize its potential \cite{ullah2025lora}. BERTweet outperformed standard BERT by 0.005 on the Kaggle leaderboard. Despite BERT's higher validation score, BERTweet's domain-specific pre-training on 850M tweets proved more effective at generalizing to the informal and noisy language of disaster-related microblogging. We observed that \textbf{label smoothing} interacts differently depending on the chosen fine-tuning strategy. Applying a smoothing factor of $\varepsilon = 0.1$ yielded a consistent improvement of $+0.002$ F1 on the Kaggle leaderboard for our LoRA-based models~\cite{ullah2025lora}. Conversely, the exact same parameter slightly hurt the performance of the fully fine-tuned BERTweet model~\cite{medium_bert}. This contrasting result demonstrates that explicit regularization is far more beneficial when the number of trainable parameters is strictly limited. Finally, our \textbf{back-translation augmentation experiments} revealed contrasting effects on model performance. While augmented data helped full fine-tuning act as a regularizer against its massive degree of freedom, it actively degraded the performance of the LoRA models. Because LoRA's capacity is constrained by its low-rank matrices, it requires high-signal data to learn efficiently; the slight semantic drift and grammatical noise introduced by the back-translation process "\textit{polluted}" LoRA's limited trainable parameters, resulting in a less precise model than one trained solely on pristine data.
